...
**关键发现**: 符号表值被加载后**在两条路径中都被立即覆盖**

**可能用途分析**:

| 可能用途 | 可能性 | 说明 |
|----------|--------|------|
| 设置 CPU 标志位 (N 标志) | 🟡 中 | ldrsb 设置负数标志，用于条件判断 |
| 内存对齐/缓存预热 | 🟢 低 | 预加载内存到缓存 |
| 调试/遗留代码 | 🟢 低 | 开发时使用的调试代码 |

**最可能用途**: `ldrsb` 指令的主要作用是**设置 CPU 标志位** (特别是 N 标志表示负数)，而不是使用加载的值本身。
...
**疑问**: 这是设计意图还是副作用？需要追踪所有对符号表区域的写操作。

### 问题 3: 符号表数据差异

**不同字符的符号表内容**:

| 字符 | r5 | r4 地址 | 符号表数据 | 分析 |
|------|----|----|-----------|------|
| 沨 | 0x0FDE | 0x1FBC0 | cd de cd df... | 有效数据 |
| 沤 | 0x0FDB | 0x1FB60 | 全零 | 可能使用默认值 |
| 沦/沪 | 0x0FDC | 0x1FB80 | 全零 | 可能使用默认值 |

**疑问**: 为什么某些字符有有效符号表数据，而其他字符是全零？
