...
## 静态分析局限性 - ×33 验证案例 (2026-01-30)

### 案例背景

在验证 ×33 计算机制时，初步分析认为 0x02DDE2/0x02DDE6 处的代码是"死代码"，因为 0x02DDE0 包含无条件跳转 `b #0x2de3c`。这个评估后来被证明是**过早的结论**。
...
0x02DDE0: b #0x2de3c           ← 无条件跳转
0x02DDE2: lsls r0, r5, #5      ← ×32 计算 (似乎被跳过)
0x02DDE6: adds r0, r4, r0       ← ×33 计算 (似乎被跳过)
```

**第二步：错误推断**
```
0x02DDE0 是无条件跳转
→ 跳过了 0x02DDE2 和 0x02DDE6
→ ×33 计算代码不执行
→ 这是死代码 ❌
```

**问题**：
1. 只分析了单一路径 (0x02DDDC → 0x02DDE0 → 0x02DE3C)
2. 没有检查其他可能的入口点
3. 忽略了条件分支可能创建的循环结构
...
0x02DDE4: bgt #0x2ddd8         ← 条件分支！
0x02DDE6: adds r0, r4, r0       ; ×33 完成
```

**第二步：分析循环结构**
```
路径 1: 0x02DDD8 → 0x02DDDC → 0x02DDE0 → 0x02DE3C (直接路径)
路径 2: 0x02DDD8 → 0x02DDDC → 0x02DDE2 → 0x02DDE4 → 0x02DDD8 (循环)
                     ↓ (如果不跳转)
                  0x02DDE6 (×33 完成)
```
...
**结论**: BX、LDR PC、MOV PC 等间接跳转的目标在运行时决定，静态分析无法追踪。
...
```assembly
; 多入口点函数
Entry1: 0x02DDDC: push {r3, r6, lr}
        0x02DDE0: b #0x2de3c     ← 快速路径

Entry2: 0x02DDD8: [前置条件检查]
        0x02DDDC: push {r3, r6, lr}
        0x02DDE2: lsls r0, r5, #5  ← 完整计算
        0x02DDE4: bgt #0x2ddd8     ← 循环条件
...
- [ ] 搜索所有可能的入口点 (±4KB 范围)
- [ ] 检查条件分支是否创建可达路径
- [ ] 分析是否有间接跳转 (BX/LDR PC/MOV PC)
- [ ] 考虑尾调用优化的可能性
- [ ] 使用动态分析验证（如果可能）

**如果以上任何一项答案不确定，不要轻易断言为"死代码"**。
...
- [执行摘要](./EXECUTIVE_SUMMARY.md) - 当前待解决问题
- [×33 计算验证](./MATH_X33_VERIFICATION.md) - 多入口点验证案例
